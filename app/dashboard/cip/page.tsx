...
export default function CIPPage() {
  ...
  // Sorting
  type SortKey = "chrTicketNumbers" | "clientName" | "cipType" | "cipStatus" | "submissionDate" | "emergencyFlag";
  ...
  const handleExportCSV = () => {
    const headers = ["#", "CHR Ticket #", "Client Name", "CIP Type", "Status", "Submission Date", "Emergency"];
  ...
  useEffect(() => { setPage(1); }, [filterStatus, filterType, filterClient, filterEmergency, dateRange]);
  ...
  // Client counts for dropdown labels
  const clientCounts  = cipRecords.reduce<Record<string, number>>((acc, r) => {
  ...
  const filteredCIP = cipRecords.filter((r) => {
    ...
    return matchStatus && matchType && matchClient && matchEmergency && matchFrom && matchTo;
  });
                ...
                {pageRecords.map((record, idx) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 max-w-[200px]">
                      <span className="block truncate text-gray-200" title={record.clientName || undefined}>
                        {record.clientName || "—"}
                      </span>
                    </td>
