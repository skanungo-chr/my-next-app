...
// ─── Normalize TFS numbers ────────────────────────────────────────────────────
// Accepts "85829", "TFS-85829", "TFS85829" → always returns "TFS-85829" or null

function normalizeTFS(raw: string): string | null {
  const str = raw.trim();
  if (!str) return null;
  // Already "TFS-NNNNN"
  if (/^TFS-\d+$/i.test(str)) return `TFS-${str.replace(/^TFS-/i, "")}`;
  // Plain digits
  if (/^\d{4,8}$/.test(str)) return `TFS-${str}`;
  // Embedded digits (e.g. "TFS85829" or "Bug 85829")
  const m = str.match(/\d{4,8}/);
  return m ? `TFS-${m[0]}` : null;
}
...
export default function CIPsByTypePage() {
  ...
  useEffect(() => { loadCIP(); }, [loadCIP]);

  const handleSync = useCallback(() => loadCIP(), [loadCIP]);
  ...
  const filtered = useMemo(() => cipRecords.filter(r => {
  ...
  }), [cipRecords, selectedType, selectedClient, fromDate, toDate]);

  // ── Group by devOpsWorkItem ─────────────────────────────────────────────────

  const tfsGroups = useMemo((): TFSGroup[] => {
    const byTFS = new Map<string, CIPRecord[]>();
    const noTFS: CIPRecord[] = [];

    for (const cip of filtered) {
      const key = normalizeTFS(cip.devOpsWorkItem ?? "");
      if (!key) {
        noTFS.push(cip);
      } else {
        if (!byTFS.has(key)) byTFS.set(key, []);
        byTFS.get(key)!.push(cip);
      }
    }

    const groups: TFSGroup[] = [];
    for (const [tfsNumber, recs] of byTFS.entries()) {
      groups.push({
        tfsNumber,
        count:   recs.length,
        types:   [...new Set(recs.map(c => c.cipType).filter(Boolean))].sort(),
        incNums: recs.map(c => c.chrTicketNumbers).filter(Boolean),
        records: recs,
      });
    }
    ...
    if (noTFS.length > 0) {
      groups.push({
        ...
        count:     noTFS.length,
        types:     [...new Set(noTFS.map(c => c.cipType).filter(Boolean))].sort(),
        incNums:   noTFS.map(c => c.chrTicketNumbers).filter(Boolean),
        records:   noTFS,
      });
    }
  ...
  // Count CIPs with no TFS number (for informational display)
  const noTFSCount = useMemo(
    () => tfsGroups.find(g => g.tfsNumber === "(No TFS Linked)")?.count ?? 0,
    [tfsGroups]
  );
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          >
            <svg
              className={`w-4 h-4 ${cipLoading ? "animate-spin" : ""}`}
              fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}
            >
              <path strokeLinecap="round" strokeLinejoin="round"
                d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
            </svg>
            {cipLoading ? "Loading…" : "Sync from TFS"}
          </button>
        </div>
      </div>
          ...
          <>
            ...
            <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-xl p-4">
              ...
              <p className="text-2xl font-bold text-indigo-400 tabular-nums">
                {tfsGroups.filter(g => g.tfsNumber !== "(No TFS Linked)").length}
              </p>
            </div>
            <div className="bg-orange-500/10 border border-orange-500/20 rounded-xl p-4">
              ...
              <p className="text-sm font-bold font-mono text-orange-400 truncate leading-tight mt-1"
                title={visibleGroups.find(g => g.tfsNumber !== "(No TFS Linked)")?.tfsNumber}>
                {visibleGroups.find(g => g.tfsNumber !== "(No TFS Linked)")?.tfsNumber ?? "—"}
              </p>
            </div>
            <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-4">
              <p className="text-xs text-gray-500 mb-1">Max Incidents</p>
              <p className="text-2xl font-bold text-green-400 tabular-nums">
                {visibleGroups.find(g => g.tfsNumber !== "(No TFS Linked)")?.count.toLocaleString() ?? "—"}
              </p>
            </div>
          </>
        )}
      ...
      <div className="space-y-5">
        ...
        {/* TFS Breakdown table */}
        {!cipLoading && visibleGroups.length > 0 && (
                ...
                {visibleGroups.map((row, i) => {
                  ...
                  return (
                    <Fragment key={row.tfsNumber}>
                      <tr
                        onClick={() => toggleRow(row.tfsNumber)}
                        className={`transition-colors select-none cursor-pointer hover:bg-amber-900/5
